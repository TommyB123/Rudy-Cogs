...
class LoggingCog(commands.Cog, name="Logging"):
    ...
    async def on_message_delete(self, message):
        ...
        if message.channel.id not in rcrp_utility.staffchannels and message.guild is not None:
            deletechan = client.get_channel(rcrp_utility.deletelogs)
    ...
    async def on_message_edit(self, before, after):
        if before.channel.id not in rcrp_utility.staffchannels and before.guild is not None:
            if before.content == after.content:
                return

            editchan = client.get_channel(rcrp_utility.editlogs)
